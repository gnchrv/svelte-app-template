...
Если `subfolder` представляет собой не виртуальный адрес, а физическую папку, необходимо также добавить в файл .env переменную `FOLDER_BASE="/subfolder/"` (со слешеми в начале и в конце) для корректного подключения статических файлов.
